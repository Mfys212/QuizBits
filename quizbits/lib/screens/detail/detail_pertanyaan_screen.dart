...
class DetailPertanyaanScreen extends StatefulWidget {
  ...
  const DetailPertanyaanScreen({super.key, required this.question});
  @override
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                ...
                child: Text(q.category, style: GoogleFonts.poppins(fontSize: 10, color: AppColors.primary, fontWeight: FontWeight.w500)),
              ),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: AppColors.success.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8)),
                child: Text(q.difficulty, style: GoogleFonts.poppins(fontSize: 10, color: AppColors.success, fontWeight: FontWeight.w500)),
              ),
              const Spacer(),
              GestureDetector(
          ...
          const SizedBox(height: 12),
          Text(q.title, style: GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 18)), // biar ngikut tema
          const SizedBox(height: 8),
          Row(
            children: [
              const Icon(Icons.visibility_outlined, size: 14, color: AppColors.textMuted),
              const SizedBox(width: 4),
              Text('${q.viewsCount} views', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textMuted)),
              const SizedBox(width: 12),
              const Icon(Icons.star, size: 14, color: AppColors.warning),
              const SizedBox(width: 4),
              Text('${q.rating} (${q.ratingCount} rating)', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textMuted)),
            ],
          ),
          const SizedBox(height: 6),
          Text('oleh ${q.authorName} • ${q.createdAt}', style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textMuted)),
          const SizedBox(height: 12),
          InkWell(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text('${q.commentsCount} Komentar', style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w500)),
                  const Spacer(),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: selectedOptionIndex == null
                 ? null
                  : () {
                      final isCorrect = selectedOptionIndex == q.correctAnswerIndex;
                      Navigator.push(context, MaterialPageRoute(builder: (_) => HasilJawabanScreen(isCorrect: isCorrect)));
                    },
              ...
              child: Text('Submit Jawaban', style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14, color: Colors.white)),
            ),
          ),
        ],
      ),
